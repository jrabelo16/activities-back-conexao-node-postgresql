...
const cadastrarUsuario = async (req, res) => {
    ...
    try {
        const usuario = conexao.query('select * from usuarios where email = $1', [email]);
        if (usuario.rowCount > 0) {
...
const atualizarUsuario = async (req, res) => {
    ...
    try {
        ...
        const query = 'update usuarios set nome = $1, idade = $2, email = $3, telefone = $4, cpf = $5 where id = $3';
...
const excluirUsuario = async (req, res) => {
    ...
    try {
        ...
        for (let emprestimo of emprestimos) {
            if (emprestimo.id_usuario === id) {
                return res.status(400).json('O usuário não pode ser excluído pois tem algum empréstimo em seu nome.')
            }
        }
